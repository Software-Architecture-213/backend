...
public class UserService {
    ...
    public UserInfoResponse disableUser(DisableUserRequest userRequest) {
        UserInfoResponse userInfoResponse = firebaseUserClient.disableUser(userRequest.getEmail(), userRequest.isDisabled());
        String subject = !userRequest.isDisabled() ? "Your account has been activated!" : "Your account has been deactivated!";
        String message = userRequest.getMessage();
        if (userRequest.getMessage() == null || message.isBlank()) {
            message = !userRequest.isDisabled()
                    ? "Dear " + userRequest.getEmail() + ", we are pleased to inform you that your account has been successfully activated. We look forward to having you use our app. If you have any questions or need assistance, feel free to reach out to our support team."
                    : "Dear " + userRequest.getEmail() + ", we would like to inform you that your account has been deactivated due to a violation of our policies. If you believe this action was taken in error, please contact our support team for further assistance. We value your experience and hope to resolve any misunderstandings.";
        }
